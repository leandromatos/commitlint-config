...
Publishing runs over OIDC (trusted publishing), with no npm token. It needs a trusted publisher registered once on npm — package settings → Trusted Publisher → GitHub Actions, pointing at `leandromatos` / `commitlint-config` / `deploy.yaml`. Without it the publish fails with a 404.
