...
This repository follows conventional commits validated by its own rules, lints with the shared ESLint config, and formats with Prettier.

## Local setup

```bash
git clone git@github.com:leandromatos/commitlint-config.git
...
- **`commit-msg`** runs this package's own rules on the message.
...
| Script                | Purpose                                       |
| --------------------- | --------------------------------------------- |
| `pnpm run build`      | Clean and compile with `tsc` and `tsc-alias`. |
...
Publishing runs over OIDC (trusted publishing), with no npm token. It needs a trusted publisher registered once on npm, under package settings → Trusted Publisher → GitHub Actions, pointing at `leandromatos` / `commitlint-config` / `deploy.yaml`. Without it the publish fails with a 404.
